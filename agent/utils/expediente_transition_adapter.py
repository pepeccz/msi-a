...
def canonicalize_transition(tool_result: Dict[str, Any]) -> CanonicalTransition:
    ...
    normalized: Dict[str, Optional[str]] = {
        channel: _normalize_sub_mode(value)
        for channel, value in raw_signals.items()
    }
    
    # Find the winner by precedence (first non-None normalized value)
    channels_precedence = ["_context_updates", "_internal_flags", "fsm_state_update", "next_step"]
    ...
    if conflicts:
        logger.warning(
            ...
            conflicting_values={
                ch: normalized[ch] for ch in conflicts
            },
            raw_signals={
