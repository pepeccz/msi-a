...
class TestHandleValidationRetry:
    ...
    def test_should_retry_when_under_limit(self, base_mode_node, fallback_handler):
        ...
        assert len(llm_messages) == 1  # Reprompt added
        assert "system" in llm_messages[0]["role"]
    ...
    def test_records_validation_context(self, base_mode_node, fallback_handler):
        ...
        assert last_context["errors"] == error_dict["validation_errors"]  # Correct field name
...
class TestFallbackHandlerGetValidationReprompt:
    ...
    def test_generates_reprompt_for_retry_2(self, fallback_handler, retry_policy):
        ...
        retry_state["last_validation_context"] = {
            "tool_name": "calcular_tarifa",
            "errors": ["Invalid categoria_slug", "Missing element"],  # Correct field name
            "layer": "semantic"
        }
    ...
    def test_generates_escalation_message_for_max_retries(self, fallback_handler, retry_policy):
...
class TestRetryFlowIntegration:
    ...
    def test_retry_flow_success_on_second_attempt(self, base_mode_node, fallback_handler):
    ...
    def test_retry_flow_escalation_after_max_retries(self, base_mode_node, fallback_handler):
