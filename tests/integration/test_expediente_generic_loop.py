...
# ---------------------------------------------------------------------------
# Test 4 — Fallback to loop_engine when USE_GENERIC_LOOP=False
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fallback_to_loop_engine_when_flag_false():
    """
    When USE_GENERIC_LOOP=False, ExpedienteModeNode should NOT call
    generic_llm_loop. The old ExpedienteLoopEngine should still handle the request.
    """
    from agent.modes.expediente_mode import ExpedienteModeNode
    ...
    # Mock the old loop engine to return a valid result
    old_loop_result = {
        "ai_response": "¿Cuál es tu nombre completo?",
    ...
    with (
        ...
        patch(
            "agent.modes.expediente_mode.generic_llm_loop",
            new_callable=AsyncMock,
        ) as mock_generic_loop,
        patch.object(
            node._loop_engine,
            "run",
            new_callable=AsyncMock,
            return_value=old_loop_result,
        ) as mock_old_loop,
    ):
        result = await node._process_message("Hola", state)

    # generic_llm_loop MUST NOT have been called
    mock_generic_loop.assert_not_called()

    # The old loop_engine.run MUST have been called
    mock_old_loop.assert_called_once()

    # The result should have an ai_response from the old loop
    assert result.get("ai_response"), (
        "Old loop_engine should have produced an ai_response"
    )
    assert result.get("ai_response") == "¿Cuál es tu nombre completo?"
