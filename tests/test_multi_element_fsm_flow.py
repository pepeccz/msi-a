...
async def test_two_element_flow_completes_and_transitions(
    ...
    with (
        patch("agent.tools.case_tools.set_current_state") as mock_set,
        ...
        assert "fsm_state_update" in result1, (
            "iniciar_expediente must return fsm_state_update"
        )
        ...
        assert "fsm_state_update" in result2, (
            "confirmar_fotos_elemento must return fsm_state_update"
        )
        ...
        assert "fsm_state_update" in result4, (
            "completar_elemento_actual MUST return fsm_state_update (NOT fsm_state)"
        )
        ...
        assert case_state["current_element_index"] == 1, (
            "Should increment to element 1 (MANILLAR)"
        )
        assert case_state["element_phase"] == "photos", (
            "Should reset to photos phase for next element"
        )
        assert case_state["element_data_status"]["SUBCHASIS"] == "complete"
        assert case_state["element_data_status"]["MANILLAR"] == "pending"
        assert (
            case_state["step"] == CollectionStep.COLLECT_ELEMENT_DATA.value
        )  # Still collecting elements
        ...
        assert "fsm_state_update" in result7, (
            "completar_elemento_actual MUST return fsm_state_update"
        )
...
async def test_completar_elemento_actual_returns_fsm_state_update_key():
    ...
    with (
        patch("agent.tools.element_data_tools.get_current_state") as mock_get,
